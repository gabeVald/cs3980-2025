...
max_id: int = (
    0  # need to get rid of this, so use either get max ID from database to get this when the db starts, or mongodbObjectID. Look at movies example in his demo.
)
...
async def add_todo(todo: TodoRequest) -> Todo:
    ...
    await Todo.insert_one(
        newTodo
    )  # since classname is inherited from document class, we can connect to db directly from Todo.mongo_commands
    return newTodo
...
@todo_router.get(
    "/{id}"
)  # Look at movies example to see authentication for user specific id
